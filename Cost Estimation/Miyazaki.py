...
def readFile():
    for row in arff.load('Cost Estimation\\Data\\miyazaki94.arff'):
        dataLine = list()
        ...
        # Adds the last value from the row (The MM Value) to the output list (The mm list)
        mm.append(row[8])
...
def initialize():
    ...
    def evalSymbReg(individual):
        ...
        for i in range(0, len(inputSet)):
            sqerrors.append(abs(func(*inputSet[i]) - mm[i]))
...
def runSymbolicRegression():
    ...
    algorithms.eaSimple(pop, toolbox, 0.8, 0.2, 1500, stats=mstats, halloffame=hof)
