...
def readFile():
    for row in arff.load('Cost Estimation\\Data\\china.arff'):
        dataLine = list()
        ...
        dataLine.append(row[7])
        dataLine.append(row[8])
        dataLine.append(row[9])
        dataLine.append(row[10])
        dataLine.append(row[11])
        dataLine.append(row[12])
        dataLine.append(row[13])
        dataLine.append(row[14])
        dataLine.append(row[15])
        dataLine.append(row[16])
        dataLine.append(row[17])
        # Adds the last value from the row (The effort Value) to the output list (The effort list)
        effort.append(row[18])
...
# This method is added because the code would crash after 500 generations because of the size of the data.
def safeFSum(x):
    try:
        return math.fsum(x)
    except OverflowError:
...
def initialize():
    ...
    # The following code is the Fitness function calculation
    def evalSymbReg(individual):
        ...
        for i in range(0, len(inputSet)):
            sqerrors.append(abs(func(*inputSet[i]) - effort[i]))
        # This is for the bigger data cases, chanced from math to sum for errors
        return safeFSum(sqerrors),
...
def runSymbolicRegression():
    ...
    algorithms.eaSimple(pop, toolbox, 0.9, 0.2, 2000, stats=mstats, halloffame=hof)
    ...
    for i in range(0, len(testSuite)):
        ...
        testSuiteMAE = testSuiteMAE + testMAE - effort[effort.__len__()-1-i]
        # Print the last 2 entries of the list of output to use as comparison against the evaluated output
        print("The actual effort is: ", effort[effort.__len__()-1-i])
